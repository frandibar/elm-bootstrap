    ...
    in
        describe "Plain progress bar"
            ...
            , test "expect div with progress-bar class and progressbar role" <|
                \() ->
                    html
                        |> Query.has [ tag "div", class "progress-bar", attribute <| HA.attribute "role" "progressbar" ]
            , test "expect minimum = 0, maximum = 100" <|
                \() ->
                    html
                        |> Query.has
                            [ attribute <| HA.attribute "aria-valuemin" "0"
                            , attribute <| HA.attribute "aria-valuemax" "100"
                            ]
            , fuzz (Fuzz.floatRange 0 100) "expect the progress value is present" <|
                \progressValue ->
                    Progress.progress [ Progress.value progressValue ]
                        ...
                        |> Query.has [ attribute <| HA.attribute "aria-value-now" (toString progressValue) ]
            ]
